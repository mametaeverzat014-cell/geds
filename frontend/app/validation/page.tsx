...
interface CascadeValidation {
  ...
  expansion: {
    ...
    v3_reached: number;
    v3_nodes: number;
    per_event: Array<{ slug: string; v2: string; v3: string }>;
    chokepoint_events_v2_only: string[];
  };
}
...
export default function ValidationPage() {
  ...
  return (
    <main className="max-w-[1600px] mx-auto px-6 py-6 space-y-4">
      ...
      {error && <div className="text-[13px] text-sev-5">{error}</div>}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {research.top_cascading_criticality_nodes.map((n) => (
                  <span key={n.node_id} className="text-[12px] px-2 py-0.5 rounded bg-bg-base/60 border border-border-subtle num">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {research.shock_absorption_capacity_summary.most_fragile_nodes.map((n) => (
                  <span key={n.node_id} className="text-[12px] px-2 py-0.5 rounded bg-sev-4/10 text-sev-4 border border-sev-4/30 num">
                ...
                <tbody>
                  {research.statistical_evaluation.map((e, i) => (
                    <tr key={i} className="border-t border-border-subtle/30">
                      <td className="py-1 num text-text-primary text-[12px]">{e.metric_name}</td>
                ...
                {bench.models.map((m) => {
                  ...
                  return (
                    <tr key={m.model} className={`border-t border-border-subtle/30 ${isWinner ? "bg-accent-cyan/5" : ""}`}>
                      <td className={`py-1 text-[12px] ${isWinner ? "text-accent-cyan font-bold" : "text-text-primary"}`}>
                        {isWinner && "★ "}{m.model}
                      </td>
      ...
      <div className="panel p-4 space-y-3">
        ...
        <p className="text-[12px] text-text-muted leading-snug">
          {tr(
            "Read: the engine ranks shape well (Spearman 0.6–0.85) but under-predicts magnitude and reaches only ~38% of hit nodes — the sparse 12-country graph has no edge to many places history hit. That gap is the quantitative case for the ICIO 81×5 expansion, measured below.",
            "Вывод: движок хорошо ранжирует форму (Spearman 0.6–0.85), но недооценивает магнитуду и доходит лишь до ~38% задетых узлов — у разреженного графа из 12 стран нет рёбер ко многим местам, которые задела история. Этот разрыв — количественный аргумент за расширение ICIO 81×5, измеренный ниже.",
          )}
        </p>

        {/* ── headline: does the dense ICIO graph fix reach? ── */}
        {cascade?.expansion && (
          <div className="mt-1 rounded-md border border-accent-violet/30 bg-gradient-to-br from-accent-violet/8 to-accent-cyan/5 p-4 space-y-3">
            <div className="flex items-baseline justify-between">
              <span className="text-[13px] font-semibold uppercase tracking-wider text-text-secondary">
                {tr("ICIO expansion impact · spatial reach", "Эффект расширения ICIO · охват")}
              </span>
              <span className="text-[12px] text-text-muted num">
                {tr("on", "на")} {cascade.expansion.events_compared} {tr("common events", "общих событиях")}
              </span>
            </div>
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
              <RecallStat
                label={tr("12-country graph", "граф 12 стран")}
                sub="36 nodes · hand-authored"
                recall={cascade.expansion.v2_recall}
                detail={`${cascade.expansion.v2_reached}/${cascade.expansion.v2_nodes}`}
                tone="muted"
              />
              <div className="text-2xl text-text-muted">→</div>
              <RecallStat
                label={tr("81-economy graph", "граф 81 экономики")}
                sub="405 nodes · OECD ICIO 2019"
                recall={cascade.expansion.v3_recall}
                detail={`${cascade.expansion.v3_reached}/${cascade.expansion.v3_nodes}`}
                tone="bright"
              />
            </div>
            <p className="text-[12px] text-text-muted leading-snug">
              {tr(
                `Same engine, same shocks — only the graph changes. Grounding the topology in the OECD inter-country input-output table lifts the share of historically-hit nodes the cascade reaches from ${(cascade.expansion.v2_recall * 100).toFixed(0)}% to ${(cascade.expansion.v3_recall * 100).toFixed(0)}%. Chokepoint events are excluded (the v3 table has no chokepoint nodes).`,
                `Тот же движок, те же шоки — меняется только граф. Привязка топологии к межстрановой таблице «затраты-выпуск» ОЭСР поднимает долю исторически задетых узлов, до которых доходит каскад, с ${(cascade.expansion.v2_recall * 100).toFixed(0)}% до ${(cascade.expansion.v3_recall * 100).toFixed(0)}%. События-чокпоинты исключены (в таблице v3 нет узлов-чокпоинтов).`,
              )}
            </p>
          </div>
        )}
      ...
      <div className="panel p-4 space-y-2">
        ...
        {looDe ? (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-[13px] pt-1">
              {([
                ...
                <div key={label}>
                  <div className="text-text-muted uppercase tracking-wider text-[12px]">{label}</div>
                  <div className={`num ${star ? "text-accent-cyan font-bold" : "text-text-primary"}`}>{star && "★ "}{value}</div>
                </div>
              ))}
            </div>
            <details className="pt-1">
              <summary className="text-[12px] text-text-muted cursor-pointer hover:text-text-secondary transition">
                {tr(`Per-event predictions (${looDe.n_folds} held-out folds)`, `Прогнозы по событиям (${looDe.n_folds} отложенных фолдов)`)}
              </summary>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-1 pt-2 text-[12px]">
                {looDe.folds.map((f) => (
      ...
      <div className="panel p-4 space-y-3">
        ...
        {portwatch ? (
          <div className="space-y-2">
            {portwatch.event_checks.map((c) => (
              <div key={c.slug} className="text-[13px] flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
            ...
            <div className="text-[13px] pt-1 border-t border-border-subtle/50">
        ...
        ) : <div className="text-[13px] text-text-muted">{tr("PortWatch: loading…", "PortWatch: загрузка…")}</div>}
        {gscpi ? (
          <div className="text-[13px] pt-1 border-t border-border-subtle/50 space-y-1">
            <div>
            ...
            <div className="text-[12px] text-text-muted">{gscpi.caveat}</div>
          </div>
        ) : <div className="text-[13px] text-text-muted">{tr("GSCPI: loading…", "GSCPI: загрузка…")}</div>}
      </div>
      ...
      <div className="panel p-4 space-y-3">
        ...
        {icio ? (
          <div className="space-y-2 text-[13px]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {Object.entries(icio.summary.correlations).map(([k, v]) => (
                <div key={k} className="rounded-lg border border-border-subtle bg-bg-base/40 px-2.5 py-1.5">
                  <div className="text-[12px] uppercase tracking-wider text-text-muted truncate" title={k}>
                    {k.replace("icio_", "").replace(/_/g, " ")}
                  </div>
                  <div className="num text-text-primary">
                    ρ {v.spearman != null ? (v.spearman >= 0 ? "+" : "") + v.spearman.toFixed(2) : "—"}
                    <span className="text-text-muted text-[12px] ml-1.5">n={v.n}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="pt-1 border-t border-border-subtle/50">
              <div className="text-[12px] uppercase tracking-wider text-text-muted mb-1">
                ...
                {icio.summary.worst_disagreements.slice(0, 5).map((w) => (
                  <div key={w.edge} className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
                    ...
                    {w.mapping_caveats.length > 0 && (
                      <span className="text-[12px] text-text-muted truncate max-w-[28rem]" title={w.mapping_caveats.join("; ")}>
                        {w.mapping_caveats[0]}
                      </span>
                    )}
                  </div>
        ...
        ) : <div className="text-[13px] text-text-muted">{tr("ICIO: loading…", "ICIO: загрузка…")}</div>}
      </div>
...
function Cell({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded border border-border-subtle bg-bg-base/30 p-2">
      <div className="text-[12px] uppercase tracking-widest text-text-muted font-bold">{label}</div>
      <div className="num text-text-primary text-sm mt-0.5">{value}</div>
    </div>
  );
}

function RecallStat({ label, sub, recall, detail, tone }: {
  label: string; sub: string; recall: number; detail: string; tone: "muted" | "bright";
}) {
  const pct = Math.round(recall * 100);
  return (
    <div className="text-center space-y-1">
      <div className="text-[12px] uppercase tracking-wider text-text-secondary">{label}</div>
      <div className={`num font-extrabold leading-none ${tone === "bright" ? "text-3xl title-gradient" : "text-3xl text-text-muted"}`}>
        {pct}%
      </div>
        ...
        <div
          ...
          style={{
            ...
            background: tone === "bright"
              ? "linear-gradient(90deg, rgba(124,108,251,0.95), rgba(77,208,225,0.95))"
              : "rgba(120,130,150,0.6)",
          }}
        />
      </div>
      <div className="text-[12px] text-text-muted num">{detail} · {sub}</div>
    </div>
  );
}
